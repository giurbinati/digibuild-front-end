			...
			AuthService.login(payload)
				.then((response) => {
					...
					if (response.status === 200 || response.status === 201) {
						...
						sessionStorage.setItem("ACCESS_TOKEN_NAME", response.data.data.token);
						redirectToHome();
					}
					else {
						//console.log(response);
				...
				<Paper elelvation={2} sx={{ boxShadow: "none", padding: 5 }}>
					<form>
						<Grid container direction="column" spacing={2} >
							...
							<Grid item>
								<FormControl>
									<FormLabel id="demo-radio-buttons-group-label">Pilot</FormLabel>
									<RadioGroup
									...
									>
										<FormControlLabel value="FVH" control={<Radio />} label="FVH" />
										<FormControlLabel value="IASI&SITTA" control={<Radio />} label="IASI&SITTA" />
									</RadioGroup>
								</FormControl>
							</Grid>
							<Grid item>
								<Button onClick={handleSubmitClick} //Button onClick={redirectToHome}
									...
									disabled={!values.email || !values.password || !pilot} type="submit" fullWidth variant="contained">
									Sign In
								</Button>
							</Grid>
							{/* <Grid item>
