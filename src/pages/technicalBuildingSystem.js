...
export default function TechbicalBuildingSystem() {

    const [categories, setCategories] = useState([]);
    const [openSnackbar, setOpenSnackbar] = useState(false);
    const [snackbarMessage, setSnackbarMessage] = useState('');
    const [snackbarSeverity, setSnackbarSeverity] = useState('success'); // Stato per la severità del messaggio
        ...
        categoriesArray.forEach(category => {
            if (typeof category.tableData === 'object' && category.tableData !== null) {
                ...
                if (data.type === 'Date' && data.value && !isValidDate(data.value)) {
                    setSnackbarMessage(`${category.name} must be a date (e.g., YYYY-MM-DD).`);
                    setSnackbarSeverity('error');
    ...
    const handleConfirmChanges = async (updatedCategories) => {
        ...
        try {
            ...
            setSnackbarMessage('Update completed successfully!'); // Imposta il messaggio di successo
            setSnackbarSeverity('success'); // Cambia la severità a successo
            setOpenSnackbar(true); // Mostra lo Snackbar
        } catch (error) {
            console.error('Errore durante la richiesta PUT:', error);
            setSnackbarMessage('Error while updating data.');
            setSnackbarSeverity('error'); // Imposta la severità dell'errore
